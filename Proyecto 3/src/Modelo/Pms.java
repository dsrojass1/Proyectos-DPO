...
public class Pms extends Observable implements Serializable {
	...
	private boolean isParqueaderoPago = false;
	private boolean isParqueaderoGratuito= false;
	private boolean isPiscina= false;
	private boolean isZonasHumedas= false;
	private boolean isBbq= false;
	private boolean isWifiGratis= false;
	private boolean isRecepcion24Horas= false;
	private boolean isAdmiteMascotas= false;
	...
	public boolean crearHabitacion(String id, String descripcion, String tipo, String ubicacion, int capacidad, boolean balcon, boolean vista,
		...
		Habitacion nuevaHabitacion = new Habitacion(id, descripcion, tipo, ubicacion, capacidad, balcon, vista, cocinaIntegrada, nuevasCamas, metrosCuadrados, tieneAireAcondicionado, tieneCalefaccion, tieneTv, tieneCafetera,
	...
	, boolean isAdmiteMascotas) {
		this.isParqueaderoPago= isParqueaderoPago;
		this.isParqueaderoGratuito= isParqueaderoGratuito;
		this.isPiscina= isPiscina;
		this.isZonasHumedas= isZonasHumedas;
		this.isBbq= isBbq;
		this.isWifiGratis= isWifiGratis;
		this.isRecepcion24Horas= isRecepcion24Horas;
	...
	public String mostrarInventarioHabitaciones() {
		String tabla = "";
	    tabla += String.format("%-10s %-40s %-25s %-45s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-15s %-15s %-10s %-10s %-10s %-10s %-10s %-15s %-10s %-10s %-15s\n", 
	            "ID", "Descripcion", "Tipo", "Ubicacion", "Capacidad", "Balcon", "Vista", "Cocina", "# Camas", "Precio", "Metros Cuad.", "Aire Acond.", 
	            "Calefacción", "Tamaño Camas", "TV", "Cafetera", "Ropa Hipo.", "Plancha", "Secador", "Voltaje AC", "Tomas USB-A", "Tomas USB-C", "Desayuno");
	    for (Habitacion habitacion : this.inventarioHabitaciones) {
	        tabla += String.format("%-10s %-40s %-25s %-45s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-15s %-15s %-10s %-10s %-10s %-10s %-10s %-10s %-15s %-10s %-10s %-15s\n", 
	                habitacion.getId(), habitacion.getDescripcion(), habitacion.getTipo(), habitacion.getUbicacion(),
	                habitacion.getCapacidad(), habitacion.isBalcon() ? "Si" : "No", habitacion.isVista() ? "Si" : "No",
	                habitacion.isCocinaIntegrada() ? "Si" : "No", habitacion.getCamas().size(), habitacion.getPrecio(),
	                habitacion.getMetrosCuadrados(), habitacion.isTieneAireAcondicionado() ? "Si" : "No",
	                habitacion.isTieneCalefaccion() ? "Si" : "No", habitacion.getTamanioCamas(),
	                habitacion.isTieneTv() ? "Si" : "No", habitacion.isTieneCafetera() ? "Si" : "No",
	                habitacion.isTieneRopaHipoalergenica() ? "Si" : "No", habitacion.isTienePlancha() ? "Si" : "No",
	                habitacion.isTieneSecadorPelo() ? "Si" : "No", habitacion.getVoltajeAC(),
	                habitacion.isTieneTomasUsbA() ? "Si" : "No", habitacion.isTieneTomasUsbC() ? "Si" : "No",
	                habitacion.isIncluyeDesayuno() ? "Si" : "No");
	    }
	    tabla += "\n------------------------------------------------------\n";
	    tabla += "Características Propias del Hotel:\n";
	    tabla += ">Parqueadero pago en el hotel: ";
	    tabla += this.isParqueaderoPago? "Sí\n": "No\n";
	    tabla += ">Parqueadero gratuito en el hotel: ";
	    tabla += this.isParqueaderoGratuito? "Sí\n": "No\n";
	    tabla += ">Piscina: ";
	    tabla += this.isPiscina? "Sí\n": "No\n";
	    tabla += ">Zonas húmedas: ";
	    tabla += this.isZonasHumedas? "Sí\n": "No\n";
	    tabla += ">BBQ: ";
	    tabla += this.isBbq? "Sí\n": "No\n";
	    tabla += ">Wifi gratis: ";
	    tabla += this.isWifiGratis? "Sí\n": "No\n";
	    tabla += ">Recepción 24 horas: ";
	    tabla += this.isRecepcion24Horas? "Sí\n": "No\n";
	    tabla += ">Admite mascotas: ";
	    tabla += this.isAdmiteMascotas? "Sí\n": "No\n";
	    return tabla;
	}
