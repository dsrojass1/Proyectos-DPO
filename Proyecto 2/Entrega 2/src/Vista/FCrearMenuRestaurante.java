...
public class FCrearMenuRestaurante extends JFrame {
	...
	private JLabel lblTitle = new JLabel("Crear Menú Restaurante");
	...
	private JButton btCrearMenu = new JButton("Crear Menú");
	private JButton btVolver = new JButton("Volver");
	
	FCrearMenuRestaurante(Control control){
		...
		String [] textoLabels = {"Id del servicio:", "Precio:", "Descripción:", "Hora de inicio disponibilidad: (HH:MM)", "Hora final disponibilidad: (HH:MM)"};
		...
		this.btCrearMenu.setFont(font2);
		this.btCrearMenu.setBackground(new Color(28, 81, 223));
		this.btCrearMenu.setForeground(new Color(255, 255, 255));
		...
		pVolver.add(btCrearMenu, gbcVolver);
		gbcVolver.gridx ++;
		...
		this.btCrearMenu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				crearMenuRst();
			}
		});
	...
	protected void crearMenuRst() {
		if (control.crearMenuRst(this.tfHash.get("Id del servicio:").getText(), Integer.parseInt(this.tfHash.get("Precio:").getText()), this.tfHash.get("Descripción:").getText(), this.tfHash.get("Hora de inicio disponibilidad: (HH:MM)").getText(), this.tfHash.get("Hora final disponibilidad: (HH:MM)").getText(), cbLlevarHabitacion.isSelected())) {
			JOptionPane.showMessageDialog(this, "Menú agregado al inventario con éxito.", "Menú resturante añadida al inventario", JOptionPane.INFORMATION_MESSAGE);
		}else {
			JOptionPane.showMessageDialog(this, "Ya existe otro servicio con el mismo id.", "Error al añadir menú", JOptionPane.INFORMATION_MESSAGE);
		}
